...
|----------|----------|-------|
| Identidad y Estado en Tiempo Real | **ODS/MDM en PostgreSQL vía API Central** | Actúa como Golden Record de usuarios entre bancos, permitiendo actualizar y consultar estados operacionales en tiempo real. |
...
```mermaid
flowchart TD
    subgraph OPERACIONAL["🔄 Capa Operacional — Master Data"]
        API["API Central
(Identidad y Estados)"]
        ODS[("DB OLTP (ODS)
PostgreSQL
...
## 13. Estrategia de Unificación de Identidades (MDM)
