...
def run_api_ingestion(endpoint, **kwargs):
    ds = kwargs["ds"]
    logger.info("Starting API ingestion for endpoint='%s', date='%s'", endpoint, ds)
    try:
        ingester = APIIngester()
        count = ingester.fetch_and_upload(dataset_endpoint=endpoint, date=ds)
    except Exception as exc:
        logger.error(
            "API ingestion failed for endpoint='%s', date='%s': %s",
            endpoint,
            ds,
            exc,
        )
        raise
    logger.info(
        "API ingestion completed for endpoint='%s': %d records", endpoint, count
    )
    return count
...
def run_csv_ingestion(filename, dataset_name, **kwargs):
    ...
    logger.info(
        "Starting CSV ingestion for file='%s', dataset='%s', date='%s'",
        source_path,
        dataset_name,
        ds,
    )
    try:
        ...
        count = ingester.ingest_csv(
            file_path=source_path, dataset_name=dataset_name, date=ds
        )
    except Exception as exc:
        logger.error(
            "CSV ingestion failed for file='%s', dataset='%s', date='%s': %s",
            source_path,
            dataset_name,
            ds,
            exc,
        ...
        raise
    logger.info(
        "CSV ingestion completed for dataset='%s': %d rows", dataset_name, count
    )
    return count
...
def run_log_ingestion(**kwargs):
    ...
    logger.info("Starting log ingestion for file='%s', date='%s'", source_path, ds)
    try:
        ingester = LogIngester()
        count = ingester.parse_and_upload_logs(log_file_path=source_path, date=ds)
    except Exception as exc:
        logger.error(
            "Log ingestion failed for file='%s', date='%s': %s",
            source_path,
            ds,
            exc,
        )
        raise
    logger.info("Log ingestion completed: %d events", count)
    return count
...
with DAG(
    ...
    task_ingest_logs = PythonOperator(
        task_id="ingest_app_logs", python_callable=run_log_ingestion
    )

    # Ejecución en paralelo dentro del Clúster de Celery Workers
    [
        task_ingest_transactions,
        task_ingest_market_data,
        task_ingest_accounts,
